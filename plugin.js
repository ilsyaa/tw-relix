...
module.exports = plugin(function ({ addVariant, e }) {
	...
	addVariant('hs-light-active', ({ modifySelectors, separator }) => {
		modifySelectors(({ className }) => {
			return `.theme-light .${e(`hs-light-active${separator}${className}`)}`;
		});
	});

	addVariant('hs-dark-active', ({ modifySelectors, separator }) => {
		modifySelectors(({ className }) => {
			return `.theme-dark .${e(`hs-dark-active${separator}${className}`)}`;
		});
	});

	addVariant('hs-auto-active', ({ modifySelectors, separator }) => {
		modifySelectors(({ className }) => {
